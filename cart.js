...
function generateCartItems(cartItems){
  ...
  cartItems.forEach((item)=>{
    ...
    <div class = 'cart-item flex items-center pb-4 border-b border-gray-100'>
                ...
                <div class = 'cart-item-brand font-bold text-sm text-gray-400'>${item.make}</div>
                </div>
                <div class = 'cart-item-counter w-48 flex items-center'>
                  <div data-id = '${item.id}' class="cart-item-decrease cursor-pointer text-gray-400 bg-gray-100 rounded h-6 w-6 items-center flex justify-center hover:bg-gray-200 mr-2"><i class="fas fa-chevron-left fa-xs"></i></div>
                  <h4 class = 'text-gray-400'>x${item.quantity}</h4>
                  <div data-id = '${item.id}' class="cart-item-increase cursor-pointer text-gray-400 bg-gray-100 rounded h-6 w-6 items-center flex justify-center hover:bg-gray-200 ml-2"><i class="fas fa-chevron-right fa-xs"></i></div>
                </div>
                <div class = 'cart-item-total-cost w-48 font-bold text-sm text-gray-400'>
